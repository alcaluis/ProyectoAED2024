...
abstract: |
  Proyecto de tratamiento de datos para la asignatura de Análisis Exploratorio de Datos. El tópico es la delincuencia en España provocado por menores de entre
  14 y 17 años (ambos inclusives). En este estudio se analizarán los delitos causados con mayor frecuencia.
keywords: |
...
3. Analizar los picos de delitos (mínimos y máximos) y valores faltantes, relacionándolos con hechos reales. (Desde sucesos hasta leyes o reformas)
...
Comenzamos ejecutando "summary" del conjunto de datos, recién cargado, para analizarlo.
...
Las columnas restantes, que son de tipo "character", representan el delito en distintos niveles de clasificación. Habremos de factorizar estas variables categóricas. El uso de la función "str" o "glimpse" en este caso no nos ha aportado información adicional.
...
Primeramente comprobaremos que los valores numéricos se encuentran dentro de los rangos esperados. Utilizando la instrucción 'stopifnot' comprobando los mínimos y máximos esperados.
...
En este apartado analizaremos si hay alguna observación faltante o repetida. Es decir, para cada delito (y sus subclasificaciones) tenemos un año y una edad específica. Para ello, vamos a contar cuantas veces aparece cada delito esperando obtener un único valor que es 44 (11 años * 4 edades * cada delito en específico). Tras realizar la prueba efectivamente obtenemos dicho valor, para todos los delitos.
...
Para validar que los datos se han acondicionado correctamente vamos a sacar las filas totales originales y compararlas con nuestro dataframe haciendo agrupaciones de los datos y sumas de los valores.
...
Para el cálculo hemos omitido todos aquellos valores de la columna "total_delitos" que eran NA. Al obtener los mismos resultados podemos confirmar que han considerado como 0 aquellos valores faltantes.

### Explorar valores faltantes

Primeramente vamos a visualizar los datos faltantes de la columna "total_delitos"; pues la otra columna que tiene valores faltantes "sub_clasi_infrac" ya está justificada. A partir de aquí vamos a tratar de razonar si estos datos son faltantes debido a un factor aleatorio o hay un razonamiento detrás.
...
* El grupo de datos faltantes del final corresponden a las "faltas" que se convirtieron en "delitos", hecho que analizaremos más adelante en el análisis. Si recogemos solo las faltas y organizamos por año confirmaremos este caso.
...
Empezamos ahora con una visualización más elaborada según la clasificación viendo su evolución en el tiempo:

### Análisis univariante

```{r analisis_variables_1, fig.cap="Evolución en el tiempo de cada tipo de delito"}
...
ggplot(data = delitos_subclasi_13,
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        strip.text = element_text(size = 6))
```
Como podemos observar, los delitos contra el patrimonio más frecuentes son los robos y los hurtos.

Conviene también analizar de forma más detallada los delitos contra la libertad, pues son de los más perpetuados y cuentan con diversas subcategorías.
...
Mediante el cálculo de una matriz de correlación hemos realizado el gráfico \ref{fig:correlaciones_delitos}, que muestra todas las correlaciones entre cada tipo de delito.
...
Por último, examinaremos la evolución de cada uno de estos delitos a lo largo de los últimos diez años mediante el gráfico \ref{fig:cant_delitos_edad_delincuente}, para cada grupo de edad.
...
Analizando primeramente la cantidad de delitos sin centrarnos en la edad, podemos ver que que los delitos más relevantes son los siguientes:
...
* Faltas contras las personas. Este, como ya hemos analizado, carece de relevancia actual así que nos centraremos en los otros 3. Ya que su importancia se ve reflejada en los delitos 3 y 6.
...
* *Covid-19*. Teniendo una gran decaimiento en la cantidad de delitos en 2020-2021, como podemos apreciar en la siguiente gráfica \ref{fig:visualizacion_explicatoria_pre_3}.
* *Reforma del código penal el año 2015*. Retirando las Faltas y considerandolas delitos. Apreciándose claramente al analizar el gráfico de correlaciones \ref{fig:correlaciones_delitos}. Donde están fuertemente correlacionadas, de manera inversa, las faltas con los delitos 3 y 6.
...
Para finalizar y con el objetivo de un mayor entendimiento del tema, proporcionamos un archivo R (en /shiny/shiny_ProyectoAED2024.R) con una app realizada mediante SHINY. Ejecutando la aplicación, podréis interactuar con dos gráficos. El primero os permitirá graficar los delitos más frecuentes a partir de la edad del infractor. El segundo os ayudará a visualizar todos los delitos y sus clasificaciones a lo largo de los años.

```{r guardar_datasets}
save(list = c("delitos_por_edad", "df_delitos_menores"),
     file = "./data/delitos_shiny.Rdata")
```