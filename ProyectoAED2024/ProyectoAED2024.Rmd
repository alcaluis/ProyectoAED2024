...
Para validar que los datos se han cargado correctamente vamos a sacar las columnas totales precaculadas y compararlas con las nuestras haciendo agrupaciones de los datos.
...
df_suma_totales <- df_delitos_menores %>%
  group_by(año) %>%
  summarize(total = sum(total_delitos, na.rm=TRUE)) %>%
  select(año, total) %>%
  arrange(año)

(cbind(df_totales, df_suma_totales))
...
df_totales_infracciones <- delitos_menores_raw %>%
  ...
  arrange(delitos_n2, año)

df_suma_totales_infracciones <- df_delitos_menores %>%
  group_by(delito_falta, año) %>%
  summarize(total = sum(total_delitos, na.rm=TRUE)) %>%
  select(delito_falta, año, total) %>%
  arrange(delito_falta, año)

(cbind(df_totales_infracciones, df_suma_totales_infracciones))
...
(cbind(df_totales_delito13, df_suma_totales_delito13[-1]))
