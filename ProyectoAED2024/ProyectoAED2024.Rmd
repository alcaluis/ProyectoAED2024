...
ggplot(data = delitos_clasi, aes(x = año, y = total)) + 
  ...
  labs(title = "Delitos por clasificación", 
       x = "Año", 
       y = "Total de Delitos") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),strip.text = element_text(size = 6)) + facet_wrap(~clasi_infrac )


```
Observamos que los mas frecuentes son los delitos contra el patrimonio y el orden socioeconómico, así como los delitos por lesiones. Además se puede apreciar un aumento en estos últimos a partir del 2016. Probablemente esto se debe a la reforma del Código Penal mencionada antes, ya que a partir de este año también se reduce drásticamente el número de faltas contra las personas. Seguramente, a partir de esta reforma, muchas infracciones contra las personas que se consideraban faltas pasaron a considerarse delitos por lesiones. Este razonamiento también se aplica al decrecimiento de las faltas contra el patrimonio en contraposición al crecimiento de los delitos en este ámbito. Por otro lado, podemos remarcar un crecimiento en los delitos contra la libertad. Por lo que respecta al resto de infracciones, no se observa ninguna tendencia creciente ni decreciente a lo largo de los últimos diez años. 
...
delitos_subclasi_10 <- df_delitos_menores %>%
  ...
  summarise(total = sum(total_delitos), .groups = 'drop') %>%  # Sumar el total de infracciones
  filter(clasi_infrac == "10 Contra la intimidad, derecho a la propia imagen")  # Filtrar solo el tipo de delito deseado

print(delitos_subclasi_10)
...
ggplot(data = delitos_subclasi_10, aes(x = año, y = total, fill = sub_clasi_infrac)) + 
  geom_bar(stat = "identity") +
  labs(title = "10 contra la intimidad, derecho a la propia imagen ", 
       x = "Año", 
       y = "Total de Delitos") +
...
delitos_subclasi_18 <- df_delitos_menores %>%
  filter(!is.na(año) & !is.na(total_delitos) & !is.na(sub_clasi_infrac)) %>%  # Quitamos los NA's
  group_by(año, clasi_infrac, sub_clasi_infrac) %>%  # Agrupamos por año y subclasificación
  summarise(total = sum(total_delitos), .groups = 'drop') %>%  # Sumar el total de infracciones
  filter(clasi_infrac == "18 Falsedades")  # Filtrar solo el tipo de delito deseado

print(delitos_subclasi_18)

```

```{r}

ggplot(data = delitos_subclasi_18, aes(x = año, y = total, fill = sub_clasi_infrac)) + 
  geom_bar(stat = "identity") +
  labs(title = "18 Falsedades", 
       x = "Año", 
       y = "Total de Delitos") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        strip.text = element_text(size = 6))
...
# Nos quedamos con los delitos por subcategorías
delitos_subclasi <- df_delitos_menores %>%
  ...
  filter(!is.na(año) & !is.na(total_delitos)) %>% # Quitamos los NA's
  group_by(año, clasi_subclasi) %>%  # Agrupamos segun el tipo de infracción
  summarise(total = sum(total_delitos), .groups = 'drop')  # Sumar el total de infracciones
  

print(delitos_subclasi)
...
delitos_ancho <- delitos_subclasi %>%
  pivot_wider(names_from = clasi_subclasi, values_from = total, values_fill = list(total= 0))
print(delitos_ancho)

# Calculamos la matriz de correlación

matriz_correlacion <- delitos_ancho %>%
...
correlaciones_relevantes <- as.data.frame(as.table(matriz_correlacion)) %>%
  ...
  arrange(desc(Freq))  # Ordena por el valor absoluto de la correlación
...
# Visualizamos las correlaciones en un mapa de calor

heatmap_data <- correlaciones_relevantes %>%
  separate(Delitos, into = c("Delito1", "Delito2"), sep = " - ") %>%
  
print(heatmap_data)

ggplot(heatmap_data, aes(Delito1, Delito2, fill = Freq)) +
  geom_tile() +
  scale_fill_gradient2(low = "blue", high = "red", mid = "white", 
                       midpoint = 0, limit = c(-1, 1), space = "Lab", 
                       name="Correlación") +
  labs(title = "Mapa de Calor de Correlaciones entre Tipos de Delitos") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
graficos[[paste("Edad", edad)]] <- grafico
