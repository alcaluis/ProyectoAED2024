...
df_delitos_menores <- df_delitos_menores %>%
  ...
  mutate(id_clasi = case_when( clasi_infrac == "Faltas contra las personas"         ~ "FvsPer",
...
EvolucionTiempoSegunDelito<- ggplot( data = df_delitos_tipo, aes(x=año, y=suma_por_tipo) )+
  ...
  facet_wrap(~factor(id_clasi, levels = as.character(unique(id_clasi)) ) , scales = "free_y",ncol=4) 
...
grafico_con_leyenda <- EvolucionTiempoSegunDelito + 
  annotation_custom(
    grid::textGrob(leyenda_texto, x = 1.05, y = 0.5, just = "left", gp = grid::gpar(fontsize = 8)),
      xmin = 12, xmax = 12.5, ymin = -Inf, ymax = Inf
  )+
  coord_cartesian(clip = "off")

# Mostrar el gráfico con la leyenda
print(grafico_con_leyenda)
