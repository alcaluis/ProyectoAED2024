...
También podemos comentar que solo un diagrama muestra un outlier, y este es el asociado a
...
ggplot(total_delitos_por_edad,
  ...
  geom_bar(stat = "identity") +
  labs(title = "Distribución de Delitos por Edad",x='Edad',y='Total delitos',fill='Edad') 
```
...
delitos_por_edad_i_año <- df_delitos_menores %>%
  ...
  filter(!is.na(edad) & !is.na(total_delitos)) %>%
  # Agrupamos segun el tipo de infracción
  group_by(edad, año, clasi_subclasi) %>%
  # Sumar el total de infracciones
  summarise(total = sum(total_delitos), .groups = 'drop')
...
top_10_delitos_por_edad_i_año <- delitos_por_edad_i_año %>%
  ...
  mutate(label_clasi_subclasi = ifelse(nchar(as.character(clasi_subclasi)) > 30,
                                   paste(substring(as.character(clasi_subclasi),
                                                 1, 26),
                                       "..."),
                                   as.character(clasi_subclasi)))
...
```{r}
    ...
    ggplot(data = top_10_delitos_por_edad_i_año,
           aes(x = año, y = total, color = clasi_subclasi)) +
    geom_line() +
    geom_point() +
