...
df_delitos_tipo <- df_delitos_menores %>%
  ...
  summarize(suma_por_tipo = sum(total_delitos,na.rm=TRUE))

windows(width = 12, height = 8)
ggplot( data = df_delitos_tipo, aes(x=año, y=suma_por_tipo))+
  geom_line()+
  labs(x = "Año", y = "Suma por Tipo")+
  facet_wrap(~clasi_infrac,scales = "free_y", ncol=4)+
  theme_bw() +
...
df_totales_calculados <- df_delitos_menores %>%
  group_by(año)%>%
  summarize(sumatotalanual = sum(total_delitos,na.rm=TRUE))
...
df_delitos_edad <- df_delitos_edad %>% 
  mutate(tipo = "Delitos por Edad")

df_totales_calculados <- df_totales_calculados %>% 
  mutate(tipo = "Total Delitos")

# Combinar los dos data frames
df_combined <- bind_rows(df_delitos_edad, df_totales_calculados)



ggplot() +
  geom_line(aes(x=año, y = suma_por_edad, color = edad), data = df_delitos_edad, size = 1) +  # Delitos por edad
  geom_line(aes(x=año,y = sumatotalanual/4) , data = df_totales_calculados, size = 1, linetype = "dashed") +  # Total delitos
  labs(x = "Año", y = "Suma") +
